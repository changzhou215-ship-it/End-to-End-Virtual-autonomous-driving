...
def visualize_steering_distribution(train_data):
    """
    可视化“提供的”训练场真实分布
    :param train_data: udacity培训数据列表
    :return:
    """
    # 获取训练集的方向数据
...
def visualize_bias_parameter_effect(train_data):
    """
    可视化“bias”参数如何影响地面真值分布
    :param train_data:
    :return:
    """
    biases = np.linspace(start=0., stop=2., num=5)
    ...
    for i, ax in enumerate(axarray.ravel()):
        b = biases[i]
        x_batch, y_batch = load_data_batch(train_data, batch_size=512, augment_data=True, bias=b)
...
if __name__ == '__main__':
    ...
    for i in range(len(train_data)):
        # 把帧地址幅值到central_frame
        central_frame = cv2.imread(path.join('data', train_data[i][0]))
        # 读取帧地址的转向信息
        steering = np.float32(train_data[i][3])
        if train_data[i][0] == 'IMG\center_2019_03_26_19_39_16_411.jpg'\
                or train_data[i][0] == 'IMG\center_2019_03_26_20_34_34_853.jpg' \
                or train_data[i][0] == 'IMG\center_2019_03_26_20_37_07_925.jpg':
