...
def split_train_val(csv_driving_data, test_size=0.2):  # test_size=0.2表示切出20%的数据作为“验证集”（用来考试）
    """
    逻辑不变：读取CSV并拆分
    """
    ...
    # 4.调用train_test_split进行数据集切割 random_state=1保证每次切出来的结果都一样方便实验
...
class DrivingDataset(Dataset):  # self可以理解为这个类的钥匙
    def __init__(self, data_list, data_dir='data', augment_data=True, bias=CONFIG['bias']):
